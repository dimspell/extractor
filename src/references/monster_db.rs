    ...
    let tx = conn.transaction()?;
    {
        ...
        for monster in monsters {
            stmt.execute(params![
                ...
                if monster.known_spell_slot1 == -1 { None } else { Some(monster.known_spell_slot1) },
                if monster.known_spell_slot2 == -1 { None } else { Some(monster.known_spell_slot2) },
                if monster.known_spell_slot3 == -1 { None } else { Some(monster.known_spell_slot3) },
                monster.is_oversize,
                monster.magic_level,
