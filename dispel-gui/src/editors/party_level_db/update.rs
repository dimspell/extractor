    ...
    match message {
        PartyLevelDbEditorMessage::LoadCatalog => {
            if app.state.shared_game_path.is_empty() {
                app.state.editors.party_level_db_editor.status_msg =
                    "Please select game path first.".into();
                return Task::none();
            }
            app.state.editors.party_level_db_editor.loading_state =
        ...
        PartyLevelDbEditorMessage::CatalogLoaded(result) => {
            ...
            match result {
                Ok((npcs, refs)) => {
                    app.state.editors.party_level_db_editor.party_refs = refs;
                    app.state.editors.party_level_db_editor.catalog = Some(npcs.clone());
                    app.state.editors.party_level_db_editor.selected_npc_idx = None;
                    app.state.editors.party_level_db_level_editor.catalog = None;
                ...
                Err(e) => {
                    ...
                    app.state.editors.party_level_db_editor.status_msg = msg.clone();
                    app.state.editors.party_level_db_level_editor.status_msg = msg;
                }
            }
            Task::none()
        }
        ...
        PartyLevelDbEditorMessage::SelectNpc(npc_idx) => {
            app.state.editors.party_level_db_editor.selected_npc_idx = Some(npc_idx);
