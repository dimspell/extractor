...
pub fn view(app: &App) -> Element<'_, Message> {
    ...
    let (Some(editor), Some(spreadsheet)) = (
        app.state.editors.dialogue_script_editor.editors.get(&tab_id),
        app.state.editors.dialogue_script_editor.spreadsheets.get(&tab_id),
    ) else {
        return container(
    ...
    view_spreadsheet(
        ...
        |idx, field, value| {
            Message::dialogue_script(DialogueScriptEditorMessage::FieldChanged(idx, field, value))
        },
