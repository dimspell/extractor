...
impl App {
    pub fn view_message_scr_editor_tab(&self) -> Element<'_, Message> {
        let editor = &self.state.message_scr_editor;
        ...
        let item_list: Vec<Element<Message>> = editor
            ...
            .map(|(idx, (_, msg))| {
                ...
                let line1 = msg.line1.clone().unwrap_or_else(|| "".into());
                let preview = if line1.len() > 40 {
                    format!("{}...", &line1[..40])
                } else {
                    line1
                };
                let label = format!("[{}] {}", msg.id, preview);

                let btn = button(text(label).size(11).font(Font::MONOSPACE))
                    .width(Fill)
                    .on_press(Message::MessageScrOpSelectMessage(idx));
        ...
        if let Some(idx) = editor.selected_idx {
            if let Some((orig_idx, _msg)) = editor.filtered_messages.get(idx) {
                let orig = *orig_idx;

                detail_content.push(labeled_input("ID:", &editor.edit_id, move |v| {
                    Message::MessageScrOpFieldChanged(orig, "id".into(), v)
                }));
                detail_content.push(labeled_input("Line 1:", &editor.edit_line1, move |v| {
                    Message::MessageScrOpFieldChanged(orig, "line1".into(), v)
                }));
                detail_content.push(labeled_input("Line 2:", &editor.edit_line2, move |v| {
                    Message::MessageScrOpFieldChanged(orig, "line2".into(), v)
                }));
                detail_content.push(labeled_input("Line 3:", &editor.edit_line3, move |v| {
                    Message::MessageScrOpFieldChanged(orig, "line3".into(), v)
                }));
            }
        } else {
            detail_content.push(
        ...
        let item_list_header = row![
            ...
            horizontal_space(),
            button(text("Scan"))
                .on_press(Message::MessageScrOpLoadCatalog)
                .padding([5, 10])
        ...
        column![
            ...
            container(
                row![
                    ...
                    button(text("Save Messages"))
                        .on_press(Message::MessageScrOpSave)
