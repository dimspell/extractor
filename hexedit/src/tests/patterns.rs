...
fn test_navigate_to_pattern_moves_cursor() {
    ...
    assert_eq!(
        state.selection.cursor, 10,
        "cursor should go to pattern start"
    );
}
...
fn test_remove_pattern_at_context_menu_removes_pattern() {
    ...
    assert_eq!(
        state.patterns.len(),
        0,
        "pattern should be removed via context menu"
    );
}
...
fn test_remove_pattern_at_context_menu_clears_addr() {
    ...
    assert!(
        ...
        "context_menu_addr should be cleared after handling"
    );
}
...
fn test_build_entries_no_patterns() {
    ...
    assert!(
        entries.iter().any(
            |e| matches!(e, CtxEntry::Disabled { label, .. } if label == "Clear All Patterns")
        ),
...
fn test_build_entries_remove_pattern_enabled() {
    ...
    assert!(
        !entries
            .iter()
            .any(|e| matches!(e, CtxEntry::Disabled { label, .. } if label == "Remove Pattern")),
...
fn test_build_entries_remove_pattern_disabled() {
    ...
    assert!(
        !entries
            .iter()
            .any(|e| matches!(e, CtxEntry::Item { label, .. } if label == "Remove Pattern")),
...
fn test_build_entries_create_pattern_disabled() {
    ...
    assert!(
        entries
            .iter()
            .any(|e| matches!(e, CtxEntry::Disabled { label, .. } if label == "Create Pattern")),
...
fn test_build_entries_all_three_combinations() {
    ...
    for (
        ...
        has_pat,
        has_addr,
        expect_create_enabled,
        expect_clear_enabled,
    ...
    ) in cases
    {
        ...
        let create_enabled = entries
            .iter()
            .any(|e| matches!(e, CtxEntry::Item { label, .. } if label == "Create Pattern"));
        let create_disabled = entries
            .iter()
            .any(|e| matches!(e, CtxEntry::Disabled { label, .. } if label == "Create Pattern"));
        assert_eq!(
            create_enabled, expect_create_enabled,
            "create enabled mismatch for ({has_sel}, {has_pat}, {has_addr})"
        );
        assert_eq!(
            create_disabled, !expect_create_enabled,
            "create disabled mismatch for ({has_sel}, {has_pat}, {has_addr})"
        );
        ...
        if has_pat {
            let remove_enabled = entries
                ...
                .any(|e| matches!(e, CtxEntry::Item { label, .. } if label == "Remove Pattern"));
            assert_eq!(
                remove_enabled, expect_remove_enabled,
                "remove enabled mismatch for ({has_sel}, {has_pat}, {has_addr})"
            );
        }

        let clear_enabled = entries
            .iter()
            .any(|e| matches!(e, CtxEntry::Item { label, .. } if label == "Clear All Patterns"));
        assert_eq!(
            clear_enabled, expect_clear_enabled,
            "clear enabled mismatch for ({has_sel}, {has_pat}, {has_addr})"
        );
    }
}
